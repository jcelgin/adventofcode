...
List<List<string>> Traverse(string from, List<string> visited)
{
    ...
    foreach (var child in children)
    {
        ...
        {
            var smallVisits = visited.Where(x => x != startPoint && x == x.ToLower());

            var d = new Dictionary<string, int>();

            foreach (var k in smallVisits)
            {
                if (d.ContainsKey(k))
                {
                    d[k]++;
            ...
            var numOfTwos = d.Where(x => x.Value >= 2).ToArray();

            if (numOfTwos.Length > 1)
            {
                continue;
            }

            if (numOfTwos.Any(x => x.Key == child))
            {
                continue;
            }
        }
