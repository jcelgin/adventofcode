...
foreach(var line in lines)
{
    ...
    {
        var xArr = GetNumbersInRangeInclusive(c0.X, c1.X).ToArray();
        var yArr = GetNumbersInRangeInclusive(c0.Y, c1.Y).ToArray();

        if (xArr.Length != yArr.Length) {
            throw new Exception($"Expected equal axis progrssion on `{line}`.");
        }

        markedCoords = new Coorindate[xArr.Length];

        for (var i = 0; i < xArr.Length; i++) 
        {
            markedCoords[i] = new Coorindate(xArr[i], yArr[i]);
        }
    }
